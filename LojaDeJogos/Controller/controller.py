...
class Controller:
    ...
    def finalizar_carrinho(self):
        ...
        if total == 0:
            ...
            return
        if self.saldo_model.desconto_saldo(total):
            ...
            print("Saldo insuficiente para a finalização da compra!")
    ...
    def adicao_saldo(self, valor, forma_pagamento):
        try:
            ...
            self.mongo.salvar_saldo(self.saldo_model.get_saldo())
            print(f"Saldo adicionado: R$ {valor:.2f} via {forma_pagamento}")
            self.mostrar_inicio()
        except ValueError:
